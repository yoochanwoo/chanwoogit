...
dfs = pd.read_csv('./1.inputdata/tn_visit_area_info_E.csv')
...
fdf.to_csv('./2.preprocessed/pre_capital.csv', encoding='cp949')

dfc = pd.read_csv('./2.preprocessed/pre_capital.csv', encoding='cp949') # 전처리된 파일 불러오기

# 전처리에 필요한 데이터만 가져와 특정 변수에 저장
dfc = dfc[['TRAVEL_ID', 'VISIT_AREA_NM','DGSTFN']]

dfc.columns = ['userID', 'itemID', 'rating']

# 결측치 확인 후 제거
dfc.isna().value_counts()

dfc2=dfc.dropna(axis=0, how='any')

# Surprise을 이용한 추천시스템


reader = Reader(rating_scale=(1, 5))
data = Dataset.load_from_df(dfc2[['userID', 'itemID', 'rating']], reader)

print(f'전체 데이터셋 수 : {dfc2.shape}')
...
# 모델 저장
with open('./4.모델저장/model/latent_model_capital.pkl','wb') as fw:
    pickle.dump(predictions, fw)

# 최종결과값 도출과정

dataset = dfc2.values.tolist()
...
user_item_matrix = dfc2.pivot_table(index = 'userID', columns = 'itemID', values = 'rating').fillna(0)
...
top_recommendations.to_csv('./4.SaveModel/result/result_capital_area.csv', index=False, encoding='cp949')
