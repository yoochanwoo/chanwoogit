...
dfe = pd.read_csv('./1.inputdata/tn_visit_area_info_F.csv')

dff = dfe.dropna(subset = ['LODGING_TYPE_CD'])  # 숙박 유형이 없는 변수 제거
...
fdf.to_csv('./2.preprocessed/pre_east.csv', encoding='cp949')

dfe = pd.read_csv('./2.preprocessed/pre_east.csv', encoding='cp949') # 전처리된 파일 불러오기

# 전처리에 필요한 데이터만 가져와 특정 변수에 저장
dfe = dfe[['TRAVEL_ID', 'VISIT_AREA_NM','DGSTFN']]

dfe.columns = ['userID', 'itemID', 'rating']

# 결측치 확인 후 제거
dfe.isna().value_counts()

dfe2=dfe.dropna(axis=0, how='any')

print(f'전체 데이터셋 수 : {dfe2.shape}')

# Surprise을 이용한 추천시스템

reader = Reader(rating_scale=(1, 5))
data = Dataset.load_from_df(dfe2[['userID', 'itemID', 'rating']], reader)
...
# 모델 저장
with open('./4.모델저장/model/latent_model_east.pkl','wb') as fw:
    pickle.dump(predictions, fw)


# 최종결과값 도출과정

dataset = dfe2.values.tolist()

predictions = algo.test(dataset)

# 예측 결과를 데이터프레임으로 변환
result_dfe = pd.DataFrame(predictions, columns=['user_id', 'item_id', 'actual_rating', 'predicted_rating', 'details'])

# 필요한 컬럼만 선택 (user_id, item_id, actual_rating, predicted_rating)
result_dfe = result_dfe[['user_id', 'item_id', 'actual_rating', 'predicted_rating']]

dfe_pivot=result_dfe.pivot_table('predicted_rating', index='user_id', columns='item_id').fillna(0)

user_item_matrix = dfe2.pivot_table(index = 'userID', columns = 'itemID', values = 'rating').fillna(0)
...
for idx, user in enumerate(user_item_matrix.index):
    ...
    sorted_acc_indices = dfe_pivot.iloc[idx].argsort()[::-1]
...
top_recommendations.to_csv('./4.SaveModel/result/result_east.csv', index=False, encoding='cp949')
